---
layout: page
title: NEATH
permalink: /neath/
---
NEATH (Non-Equilibrium Abundances Treated Holistically) is a framework for post-processing hydrodynamical simulations with a time-dependent chemical network. It has been calibrated to return abundances consistent with the underlying hydro simulations for species which are important for the thermal evolution of the gas. The abundances of hundreds of additional species, many of which are important observational tracers of star-forming regions, are also calculated at a low computational cost. Details of the NEATH framework are described in [this paper](https://ui.adsabs.harvard.edu/abs/2023MNRAS.524.5971P). Source code used for the chemical modelling and example data are available [here](https://github.com/fpriestley/neath/).

### Publications

[NEATH ─ V. The relationship between line emission from dense gas tracers and the star formation rate](https://ui.adsabs.harvard.edu/abs/2026MNRAS.547ag527P/abstract)

[NEATH IV: an early onset of complex organic chemistry in molecular clouds](https://ui.adsabs.harvard.edu/abs/2025MNRAS.537.2453P/abstract)

[NEATH - III. A molecular line survey of a simulated star-forming cloud](https://ui.adsabs.harvard.edu/abs/2024MNRAS.531.4408P/abstract) (data available [here](https://cf-my.sharepoint.com/personal/priestleyf_cardiff_ac_uk/_layouts/15/onedrive.aspx?id=%2Fpersonal%2Fpriestleyf%5Fcardiff%5Fac%5Fuk%2FDocuments%2Fsurvey&ct=1776841478850&or=OWA%2DNTB%2DMail))

[NEATH - II. N2H+ as a tracer of imminent star formation in quiescent high-density gas](https://ui.adsabs.harvard.edu/abs/2023MNRAS.526.4952P/abstract)

[Non-Equilibrium Abundances Treated Holistically (NEATH): the molecular composition of star-forming clouds](https://ui.adsabs.harvard.edu/abs/2023MNRAS.524.5971P/abstract)
